...
package com.quang.lilianachat.ui.wheelview;
...
import com.quang.lilianachat.ui.wheelview.adapter.WheelAdapter;
import com.quang.lilianachat.ui.wheelview.transformer.FadingSelectionTransformer;
import com.quang.lilianachat.ui.wheelview.transformer.ScalingItemTransformer;
import com.quang.lilianachat.ui.wheelview.transformer.WheelItemTransformer;
import com.quang.lilianachat.ui.wheelview.transformer.WheelSelectionTransformer;
...
public class WheelView extends View {
    ...
    private static final float VELOCITY_FRICTION_COEFFICIENT = 0.0015f;
    private static final float CONSTANT_FRICTION_COEFFICIENT = 0.0028f;
    private static final float ANGULAR_VEL_COEFFICIENT = 25f;
    private static final float MAX_ANGULAR_VEL = 0.55f;
    ...
    private int mRawStopedPosition;
    ...
    private OnWheelStopListener mOnWheelStopListener;
    ...
    public interface OnWheelStopListener {
         ...
         * @param position     of the adapter that is closest to the selection angle
         */
        void OnWheelStoped(WheelView parent, Drawable itemDrawable, int position);

        //TODO onWheelItemSettled?
    }

    public void setOnWheelStopListener(OnWheelStopListener listener) {
        mOnWheelStopListener = listener;
    }

    public OnWheelStopListener getOnWheelStopListener() {
        return mOnWheelStopListener;
    }
...
//        Log.d("Quang", mAngularVelocity + "");
//        if (!mIsDraggingWheel) setStopedPosition(position);
    }
    ...
    private void setStopedPosition(int position) {
        if (mRawStopedPosition == position) return;

        mRawStopedPosition = position;

        if (mOnWheelStopListener != null && !isEmptyItemPosition(position)) {
            int adapterPos = getStopedPosition();
            mOnWheelStopListener.OnWheelStoped(this, getWheelItemDrawable(adapterPos), adapterPos);
        }
    }
    ...
    public int getStopedPosition() {
        return rawPositionToWheelPosition(mRawStopedPosition);
    }
    ...
    private void update(float deltaTime) {
        ...
        int position = (int) ((-mAngle + -0.5 * Math.signum(mAngle) * mItemAngle) / mItemAngle);
        if (mAngularVelocity <= 0) setStopedPosition(position);
    }
